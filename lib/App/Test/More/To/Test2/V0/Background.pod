...
B<Note> that changing C<use_ok ...> to C<use ok ...> changes the order of execution, because a C<use ...> is done at compile time. If you want to use C<plan()> with C<use ok ...>, you will need to code it as C<BEGIN { plan ...; }>.

B<Note also> that while C<use_ok( ... ) or ...> is perfectly legitimate Perl, C<use ok ... or ...> is a syntax error. If the code was in fact C<use_ok( ... ) or BAIL_OUT ...;> you will need to either abandon the C<or ...>, convert the module to L<Test2::Plugin::BailOnFail|Test2::Plugin::BailOnFail>, (see L<C<BAIL_OUT()>|/BAIL_OUT()>, above) or convert C<use_ok()> as though it were L<C<require_ok()>|/require_ok()> (see below).
...
There are several functional differences in the way the two packages handle captured warnings:

=over

=item * L<Test::Warn|Test::Warn> distinguishes between warnings from the C<warn()> built-in and those from C<Carp::carp()>. L<Test2::Tools::Warnings|Test2::Tools::Warnings> does not.

=item * L<Test::Warn|Test::Warn> does string comparisons excluding the line number and file name, if present. As far as I can tell, C<Test2-Suite> provides no such functionality.

=item * L<Test::Warn|Test::Warn> can test whether warnings came from a given L<warnings|warnings/Category Hierarchy> category. As far as I can tell, C<Test2-Suite> provides no such functionality.

=back

If there is a C<Test2::Tools> package that supplies this functionality, I have not identified it. Certainly nothing in my L<Annotated Test2::Tools Index|https://trwyant.github.io/misc/all-perl-test2-tools.html> jumps out at me.
...
=head2 Converting C<load_module()>

An option, C<--load-module>, determines which alternative is used. If this option is asserted, a C<use Test2::Tools::LoadModule ':more';> is added to provide a C<load_module()> subroutine. If not (the default), C<load_module ...> is converted to C<ok lives { require ... }>.

=head2 Converting C<use_module()>

Option C<--load-module> also controls the conversion of this subroutine. If it is asserted, it is handled the same way as L<load_module()|/Converting load_module()>, above. If it is negated, it is converted to C<use ok ...>, and any trailing C< or ...> is made into a diagnostic, since C<use ok ... or ...> is a syntax error.
